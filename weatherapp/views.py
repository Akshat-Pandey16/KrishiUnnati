...
from .weather import recursive_items
import datetime
...
def result(request):
    ...
    forecast_url = (
        "https://api.agromonitoring.com/agro/1.0/weather/forecast?lat=%s&lon=%s9&appid=%s"
        % (x, y, apikey)
    )
        ...
        for key, value in recursive_items(x):
            if key == "dt":
                ...
                dt = datetime.datetime.fromtimestamp(
                    date_json / 1000.0, tz=datetime.timezone.utc
                )
